...
- Motion is layered: content is always visible; diagrams, cases and hand-drawn DevOps paper props reveal once. The hero props can drift subtly after revealing, while the marquee remains the only high-visibility continuous motion. Motion stays within 250–600ms and moves no more than 1.25rem.
- Ambient props are semantic-free, `aria-hidden` isometric SVG sketches of actual delivery concerns: Kubernetes pods, CI runners, Tailscale mesh links, Podman/K3s/Argo delivery pieces, PostgreSQL backup and security gates. A low-opacity fixed field carries four distributed infrastructure props through the full desktop canvas. Each prop idles independently at a near-imperceptible speed, reacts to pointer movement and click impulse, and receives a short directional impulse during page scroll. Section stages retain their own layered props. Compact layouts keep one low-opacity static composition per section, while reduced motion removes every ambient layer.
